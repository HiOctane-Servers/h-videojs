class HPlayer {
    ...
    /**
     *
     * @description Initialize HPlayer
     * @example
     *      const video = document.querySelector('video');
     *      const hPlayer = new HPlayer(video);
    ...
    constructor(videoElement: HTMLVideoElement) {
        ...
        // basic dom setup
        video.parentNode?.replaceChild(playerContainer, video); // replace video element with player-container
        videoWrapper.appendChild(video); // append video element inside video-wrapper

        // set player-container dimensions as video element's
        const videoWidth = video.getAttribute('width') || '100%',
            videoHeight = video.getAttribute('height') || '100%';
        playerContainer.style.setProperty('--h-width', videoWidth);
        playerContainer.style.setProperty('--h-height', videoHeight);

        // remove video element attributes to prevent clashing
        ...
        // Element: controls-container -> main-controls
        ...
        // append everything inside player-container element
        ...
        playerContainer.appendChild(videoWrapper); // append video-wrapper
        playerContainer.appendChild(playerControls); // append player-controls
         ...
         * @returns State
         */
        const showControls = (state: boolean) => {
            const { classList } = playerControls;
            if (state == true) classList.remove('hidden');
        ...
        /**
         *
         * @param state 'play' = set state to playing, 'pause' = set state to paused
         * @returns State
         */
        const playbackState = (
            state: 'play' | 'pause' | 'loading' | 'finished'
        ) => {
            if (state != 'loading') {
                const icon = state == 'pause' ? 'play' : 'pause',
                    title = state == 'pause' ? 'Play' : 'Pause';
        ...
        var playbackPromise: any = undefined;
        /**
         *
         * @param state true = video.play(), false = video.pause()
         * @returns
         */
        const togglePlayback = (state?: boolean | undefined) => {
            const { video } = this;
        ...
        const timeString = (time: number) => {
            ...
            if (isNaN(hour + mins + secs)) return '0:00';
            ...
            const stringOutput = str.map((str, i) => {
                if (i == 0) return str != '0' ? str + ':' : undefined;
                if (i == 1) {
                    if (hour > 0) return str.padStart(2, '0') + ':';
                    return str + ':';
                }
                return str;
            });
        ...
        const playerFullscreenHandler = async (goFullscreen?: boolean) => {
            const fullscreenFunction = (fullscreen: boolean) => {
                // enter fullscreen
            ...
            if (goFullscreen) return fullscreenFunction(goFullscreen);

            // fullscreen toggle
            return fullscreenFunction(!document.fullscreenElement);
        };

        /* listen to play + pause + finished events */
        ...
        video.addEventListener('canplay', () => {
            if (video.paused) playbackState('pause');
        ...
        /* playerContainer listen to mouse events */
        ...
        playerContainer.addEventListener('mouseleave', () => {
            // do not hide player controls if video is paused
        ...
        const updateVolume = (volume: number) => {
            if (volume > 0) {
                ...
                if (volume > 5e-1) volumeButton.innerHTML = assets.icon.volume;
                if (volume < 5e-1) volumeButton.innerHTML = assets.icon.volL50;
                if (volume < 25e-2) volumeButton.innerHTML = assets.icon.volL25;
            }
            if (volume == 0) {
    ...
    /**
     *
     * @description Loads video file into the player
     * @example
     *      const hPlayer = new HPlayer(video);
     *      hPlayer.loadSrc('https://cdn.yoursite.com/video/myVideo.webm');
     * @param src Video file
     * @returns {String} Video file
     */
     ...
     *      const hPlayer = new HPlayer(video);
     *      hPlayer.play()
     * @returns Play request
     */
    play = () => this.togglePlayback(true);
     ...
     *      const hPlayer = new HPlayer(video);
     *      hPlayer.pause()
     * @returns Pause request
     */
    pause = () => this.togglePlayback(false);
}
